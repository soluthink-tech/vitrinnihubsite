...
const Ecosystem = () => {
  ...
  return (
    <section id="ecosystem" className="py-12 md:py-20 xl:py-32 relative">
      <div className="container mx-auto px-4 md:px-6 xl:px-6 relative z-10">
        ...
        {/* Apps Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 md:gap-6 xl:gap-8 max-w-6xl mx-auto">
          {apps.map((app, index) => (
            <Card 
              key={index}
            ...
            >
              ...
              <CardContent className="pt-6 md:pt-8 pb-6 md:pb-8 relative z-10">
                <div className={`w-12 h-12 md:w-16 md:h-16 rounded-2xl bg-gradient-to-br ${app.gradient} flex items-center justify-center mb-4 md:mb-6 group-hover:scale-110 transition-transform duration-500`}>
                  <app.icon className="w-6 h-6 md:w-8 md:h-8 text-white" />
                </div>
                
                <h3 className="text-xl md:text-2xl font-bold mb-2">{app.title}</h3>
                <p className="text-sm md:text-base text-muted-foreground mb-4 md:mb-6">{app.description}</p>
